...
let filter_mapi func list =
  let rec aux acc ix =
    ...
    | [] -> List.rev acc
    | x :: xs ->
      match func ix x with
      | Some y -> aux (y :: acc) (ix + 1) xs
      | None -> aux acc (ix + 1) xs
  in
  aux [] 0 list
...
let vrednost p x =
  let vals = List.mapi (fun i a -> a * (pow x i)) p in
...
let izpis p =
  ...
  let digit b n =
    ...
    | n -> (
      ...
      | 0 -> ""
      | 1 -> "x" ^ (superscript n)
      | b -> (Int.to_string b) ^ " x" ^ (superscript n)
    )
  in
  ...
  let rec aux acc n =
    ...
    | b :: bs -> (
      match b with
      | 0 -> aux acc (n - 1) bs
      | b when b < 0 -> aux (acc ^ sign acc b ^ digit (-b) n) (n - 1) bs
      | b -> aux (acc ^ sign acc b ^ digit b n) (n - 1) bs
    )
  in
...
let konstanta y : odvedljiva = ((fun _ -> y), (fun _ -> 0.))
...
let sifriraj key str =
  let replace char =
    if 0 <= indeks char && indeks char < String.length key then
      key.[indeks char]
    else
      char
  in
  String.map replace str
...
let inverz key =
  let find target =
    match String.index_opt key target with
    | Some index -> Some (crka index)
    | None -> None
  in
...
let slovar = List.map (fun word -> String.uppercase_ascii word) (String.split_on_char ' ' besede)
...
let dodaj_zamenjavo current (source, target) =
  ...
  (* Tej črki je že bila dodeljena druga zamenjava *)
  | index when current.[index] <> '_' && current.[index] <> target -> None

  (* Drugi črki je že bila dodeljena ta zamenjava *)
  | index when String.contains current target && String.index current target <> index -> None

  (* Vse je v redu *)
  | index -> Some (String.mapi (fun ix char -> if ix = index then target else char) current)
...
let dodaj_zamenjave current (source, target) =
  let rec aux current (source, target) =
    match current with
    | None -> None
    | Some current ->
      match (source, target) with
      | (s :: [], t :: []) -> dodaj_zamenjavo current (s, t)
      | (s :: ss, t :: ts) -> aux (dodaj_zamenjavo current (s, t)) (ss, ts)
      | _ -> None
  in

  let source = List.of_seq (String.to_seq source) in
  let target = List.of_seq (String.to_seq target) in
  aux (Some current) (source, target)
...
let mozne_razsiritve current encrypted dictionary =
  let aux acc plain =
    match dodaj_zamenjave current (encrypted, plain) with
    | Some extended -> extended :: acc
    | None -> acc
  in
...
let odsifriraj encrypted =
  let rec aux current =
    function
    | [] -> current
    | word :: words ->
      let extensions = List.concat_map (fun key -> mozne_razsiritve key word slovar) current in
      aux extensions words
  in

  aux [String.make 26 '_'] (String.split_on_char ' ' encrypted)
  |> List.map (fun key -> (key, sifriraj key encrypted))

(* Opomba: Zdi se mi bolj smiselno, da funkcija vrne seznam vseh možnih rešitev *)

let odsifriraj_enega encrypted =
  match odsifriraj encrypted with
  | [] -> None
  | (_, plain) :: _ -> Some plain
