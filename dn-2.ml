...
let init instructions = { empty with instructions = Array.of_list instructions }
...
let read_instruction { instructions; ip = Address ip } =
  if ip < 0 || ip >= Array.length instructions then None
  else Some instructions.(ip)
...
let read_register { a; b; c; d } register =
  match register with
  | A -> a
  | B -> b
  | C -> c
...
let primer_izvajanje_4 = read_expression { empty with a = 10; b = 20 } (Const 42)
...
let write_register state register number =
  match register with
  | A -> { state with a = number }
  | B -> { state with b = number }
  | C -> { state with c = number }
  | D -> { state with d = number }

let primer_izvajanje_5 = write_register { empty with c = 42 } D 24
...
let perform_unop operation state register =
  operation (read_register state register)
  |> write_register state register

let primer_izvajanje_6 = perform_unop (fun x -> 101 * x) { empty with c = 5 } C
...
let perform_binop operation state register expression =
  operation (read_register state register) (read_expression state expression)
  |> write_register state register

let primer_izvajanje_7 = perform_binop ( * ) { empty with c = 5 } C (Const 101)
...
let next (Address address) = Address (address + 1)

let primer_izvajanje_8 = next (Address 41)
...
let jump state address = { state with ip = address }
let proceed state = jump state (next state.ip)

let primer_izvajanje_9 = jump { empty with ip = Address 42} (Address 10)
(* val primer_izvajanje_9 : state =
  {instructions = [||]; a = 0; b = 0; c = 0; d = 0; ip = Address 10;
   zero = false; carry = false; stack = []} *)

let primer_izvajanje_10 = proceed { empty with ip = Address 42}
...
let push_stack state number = { state with stack = number :: state.stack }
...
let primer_izvajanje_11 = push_stack { empty with stack = [1; 2; 3] } 42
(* val primer_izvajanje_10 : state =
  {instructions = [||]; a = 0; b = 0; c = 0; d = 0; ip = Address 0;
   zero = false; carry = false; stack = [42; 1; 2; 3]} *)

let primer_izvajanje_11 = pop_stack { empty with stack = [1; 2; 3] }
...
let compare state first second = {
  state with
  zero = first = second;
  carry = first < second;
}

let primer_izvajanje_12 = compare empty 24 42
...
let conditional_jump state address =
  function
  | true -> jump state address
  | false -> proceed state

let primer_izvajanje_13 = conditional_jump { empty with ip = Address 42 } (Address 10) true
(* val primer_izvajanje_13 : state =
  {instructions = [||]; a = 0; b = 0; c = 0; d = 0; ip = Address 10;
   zero = false; carry = false; stack = []} *)

let primer_izvajanje_14 = conditional_jump { empty with ip = Address 42 } (Address 10) false
...
let call state address =
  let Address other = next state.ip in
  push_stack (jump state address) other

let primer_izvajanje_15 = call { empty with ip = Address 42 } (Address 10)
...
let return state =
  let (other, state) = pop_stack state in
  jump state (Address other)

let primer_izvajanje_16 = return { empty with ip = (Address 100); stack = [42; 43; 44] }
...
let rec run_program state =
  ...
  | Some HLT | None -> state
  | Some instruction -> run_instruction state instruction |> run_program
...
let parse_expression =
  function
  | r when "A" <= r && r <= "Z" -> Register (parse_register r)
...
let parse_address labels address =
  match List.assoc_opt address labels with
  | Some address -> address
  | None -> Address (int_of_string address)
...
let parse_label line =
  ...
  | true -> Some (String.sub line 0 (String.length line - 1))
  | false -> None
...
let parse_labels =
  let rec aux labels filtered =
    function
    | [] -> (labels, List.rev filtered)
    | line :: lines ->
      match parse_label line with
      | Some label -> aux ((label, Address (List.length filtered)) :: labels) filtered lines
      | None -> aux labels (line :: filtered) lines
  in aux [] []

let primer_branje_9 = parse_labels ["JMP main"; "main:"; "MOV A, 0"; "loop:"; "INC A"; "JMP loop"]
...
let primer_branje_10 = List.map (parse_instruction [("main", Address 42)]) ["MOV A, 42"; "CALL main"; "HLT"]
...
let run program =
  ...
  List.map (parse_instruction labels) instructions
  |> init
  |> run_program
...
let fibonacci2 = {|
  ...
  HLT
|}
(* val fibonacci2 : string =
  "\n  JMP main\n  ; Funkcija, ki izračuna fib(A) in vrednost shrani v register A\n  fib:\n      ; Shranimo vrednosti registrov\n      PUSH C\n      PUSH B\n  \n      ; V C shranimo začetno vrednost A\n      MOV C, A\n  \n      ; Če je A = 0, je to tudi rezultat\n      CMP A, 0\n      JE .fib_end\n  \n      ; Če"... (* string length 872; truncated *) *)

let primer_branje_11 = run fibonacci2
